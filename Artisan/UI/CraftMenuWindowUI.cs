    ...
    internal class CraftMenuWindowUI : Window
    {
        ...
        public override void Draw()
        {
            try
            {
                ...
                // Window opens whenever the recipe list panel is visible, even before any recipe row is
                // selected (Endurance.RecipeID == 0 then). LuminaSheets.RecipeSheet[Endurance.RecipeID]
                // below throws KeyNotFoundException in that case, silently swallowed by the catch at the
                // bottom of this method -- the window opened but drew almost nothing.
                if (!LuminaSheets.RecipeSheet.ContainsKey(Endurance.RecipeID))
                {
                    return;
                }
            ...
            {
                // TODO(api13): was a bare `catch { }` -- swallowed the exact exception that made this
                // window render empty. Logging it so the next failure is diagnosable instead of guessed.
                ex.Log();
            }
        }
    }
