            ...
            foreach (var questCats in Svc.Data.GetExcelSheet<Quest>().Where(x => x.JournalGenre.RowId is >= 165 and <= 172).GroupBy(x => x.JournalGenre.RowId).OrderBy(x => x.Key))
            {
                foreach (var quest in questCats.OrderBy(x => x.ClassJobLevel.First()))
                {
                    ...
                    foreach (var reqItem in reqItems)
                    {
                        ...
                        // TODO(api13): the embedded QuestRequiredItem CSV was built against international
                        // game data; some rows reference items/recipes not yet in TC's older sheet data
                        // (game 7.20). Guard instead of throwing so one missing row doesn't kill every
                        // premade list (this crashed the plugin's ctor on load before the guard was added).
                        var recipe = LuminaSheets.RecipeSheet.Values.FirstOrDefault(x => x.ItemResult.Value.RowId == reqItem.ItemId && x.CraftType.RowId == quest.JournalGenre.RowId - 165);
                        if (recipe.RowId == 0)
                        {
                            Svc.Log.Debug($"No recipe found for item {reqItem.ItemId} in {list.Name}, skipping.");
                            continue;
                        }
