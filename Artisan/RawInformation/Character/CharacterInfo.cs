...
namespace Artisan.RawInformation.Character
{
    ...
    internal unsafe class RecipeInformation : IDisposable
    {
        ...
        private List<uint> Uncompletables = new List<uint>()
        {
            ...
            30390, 30391, 30392, 30393, 30394, 30395, 30396, 30397, 30398, 30399, 30400, 30401, 38254, 38255,
            38256, 38257, 38258, 38259, 38260, 38261
        };
        ...
        {
            if (!EzThrottler.Throttle("CompletedRecipesUpdate", TimeSpan.FromSeconds(5)))
                return;
