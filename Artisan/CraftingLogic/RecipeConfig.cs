...
public class RecipeConfig
{
    ...
    public bool Draw(uint recipeId)
    {
        ...
        var liveStats = Player.JobId == craft.Recipe.CraftType.RowId + 8;
        bool changed = false;
        changed |= DrawFood();
        changed |= DrawPotion();
        changed |= DrawManual();
    ...
    public unsafe void DrawSimulator(CraftState craft)
    {
        if (!P.Config.HideRecipeWindowSimulator)
        {
            ...
            if (ImGui.IsItemClicked())
            {
                ...
                {
                    if ((Job)gs.ClassJob == Job.CRP + (byte)recipe.CraftType.RowId)
                    {
                        if (SimulatorUI.SimGS is null || (Job)SimulatorUI.SimGS.Value.ClassJob != Job.CRP + (byte)recipe.CraftType.RowId)
                        {
