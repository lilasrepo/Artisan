...
public class ItemSelector<T>
{
    ...
    public ItemSelector(IList<T> items, Flags flags = Flags.None)
    {
        ...
        _numButtons = (byte)(ListFlags & Flags.ButtonMask) switch
        {
            0x01 => 1,
            0x02 => 1,
            0x03 => 2,
    ...
    private unsafe void InternalDraw(int idx)
    {
        ...
        if (ListFlags.HasFlag(Flags.Move))
        {
            using var source = ImRaii.DragDropSource();
            if (source)
            {
        ...
        if ((ListFlags & (Flags.Move | Flags.Drop)) == Flags.None)
            return;
    ...
    {
        if (!ListFlags.HasFlag(Flags.Filter))
            return;
    ...
    private void DrawButtons(float width)
    {
        ...
        if (ListFlags.HasFlag(Flags.Add))
        {
            DrawAddButton(buttonWidth);
            ImGui.SameLine();
        }

        if (ListFlags.HasFlag(Flags.Import))
        {
            DrawImportButton(buttonWidth);
            ImGui.SameLine();
        }

        if (ListFlags.HasFlag(Flags.Duplicate))
        {
            DrawDuplicateButton(buttonWidth);
            ImGui.SameLine();
        }

        if (ListFlags.HasFlag(Flags.Delete))
        {
